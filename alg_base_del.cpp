    ...
    int alg(int net_size, vector<Node>& net) {
        ...
        for (int start_place = 0; start_place < net_size; ++start_place) {
            ...
            for (int place : res.second) {
                ...
                set<int> next_transitions = net[place].next;
                set<int> prev_transitions = net[place].prev;
                ...
                for (int next: next_transitions) {
                    net[next].prev.erase(place);
                    net[place].next.erase(next);
                }
                for (int prev: prev_transitions) {
                    net[prev].next.erase(place);
                    net[place].prev.erase(prev);
                }
            }
